...
class PolarH10:
    ...
    def __init__(self, bleak_device):
        ...
        self.ecg_stream_start_time = None
        self.acc_data = None
        self.ibi_data = None
        self.ecg_data = None
    ...
    # 00 = ECG; EA 1C AC CC 99 43 52 08 = last sample timestamp in nanoseconds; 00 = ECG frameType, sample0 = [68 00 00] microVolts(104) , sample1, sample2, ....
        if data[0] == 0x00:
            if not bool(self.ecg_stream_values):
                self.ecg_stream_start_time = time.time_ns()/1.0e9
    ...
    def get_ecg_data(self):
        ...
        ecg_times = np.array(self.ecg_stream_times)
        ecg_times = ecg_times - self.ecg_stream_start_time # rel to start of acc session time in unix s
        self.ecg_data = {'times': ecg_times, 'values': np.array(self.ecg_stream_values)}
